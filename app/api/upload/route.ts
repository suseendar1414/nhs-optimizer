...
export async function POST(request: Request) {
    ...
    const providedLocation = formData.get('homeLocation') as string;

    if (!files.length || !userId) {
        return NextResponse.json({ error: 'Files and User ID required' }, { status: 400 });
    }

    const allProcessedShifts = [];

    // Fetch user profile for home location ONLY IF providedLocation is missing
    let homeLocation = providedLocation;
    if (!homeLocation) {
        const { data: profile } = await supabase
            ...
            .eq('id', userId)
            .single();
        homeLocation = profile?.home_location || 'London, UK';
    }
    ...
    try {
        for (const file of files) {
            ...
            if (shifts.length === 0) {
                ...
                continue; // Skip to next file if none found in this one
            }
            ...
            // const processedShifts = []; // This is now `allProcessedShifts` outside the loop

            // Fetch user profile for home location for each file is not needed, moved outside the loop
            // let homeLocation = providedLocation;
            // if (!homeLocation) {
            //     const { data: profile } = await supabase
            //         .from('profiles')
            ...
            for (const shift of shifts) {
                ...
                if (!error) {
                    allProcessedShifts.push(insertedShift);
                }
            }
        }

        return NextResponse.json({ success: true, shifts: allProcessedShifts });
